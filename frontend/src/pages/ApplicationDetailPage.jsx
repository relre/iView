...
const ApplicationDetailPage = () => {
  ...
  const questionList = interviewQuestions && interviewQuestions.length > 0 ? interviewQuestions.map((question, index) => {
    ...
    const questionDuration = (question.minutes * 60) + (question.seconds || 0); // Add seconds if available
    currentTime += questionDuration;
    return (
      <li key={index}>
        <span onClick={() => startVideoAt(startTime)}  className="bg-gray-200 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded hover:cursor-pointer hover:bg-rtwgreenligth2">
          {formatTime(startTime)} - {formatTime(currentTime)}
        </span>
        ...
        <div className='w-1/2 p-6'>
          <div>
            <div className='flex items-center'>
              <PlayCircleIcon className='w-6 h-6 text-rtwgreen mr-2' /> 
              <h2 className='text-2xl'><strong>{application.name}  {' '}
              <span className='text-rtwgreendark'>{application.surname}</span></strong> </h2>
            </div>
